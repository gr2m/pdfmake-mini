 ...
 */
function PdfPrinter(doc, fontDescriptors) {
  this.doc = doc;
...
function renderLine(line, x, y, pdfKitDoc) {
  ...
  for(var i = 0, l = line.inlines.length; i < l; i++) {
    ...
    pdfKitDoc.addContent('/' + (encoded.fontId || inline.font.id) + ' ' + inline.fontSize + ' Tf');
        // pdfKitDoc.addContent('<' + encoded.encodedText + '> Tj');
        pdfKitDoc.addContent('<' + (encoded.encodedText || encoded) + '> Tj');
